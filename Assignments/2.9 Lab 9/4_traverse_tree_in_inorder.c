/*
Given a [info message="A binary tree is a tree data structure in which each node has at most two children, which are referred to as the left child and the right child. "]binary tree[/info], create an in-order  array representation of it.
...
that accept root node of binary tree R of type 'struct node'. The function should return in-order array representation of the binary tree.
Input
    3
    2 3 1
    2 1 3
...
Output
    2 1 3
Here the output is the in-order representation of the constructed tree.
Assume that, 
...
void solution(struct Node *root) 
{
    ...
    else 
    {
        ...
        printf("%d ",root->key);           // Printing the root or the last elem
