/*
Given a [info message="A binary tree is a tree data structure in which each node has at most two children, which are referred to as the left child and the right child."]binary tree[/info], create a post-order array representation of it.
...
that accept root node of binary tree R of type 'struct node'. The function should return post-order array representation of the binary tree.
Input
    3
    2 3 1
    2 1 3
...
Output
        2 3 1
Here the output is the post-order representation of the constructed tree.
Assume that, 
N is an integer within the range [0 to 10000]. 
Array elements are in the range [-2147483648 to 2147483647].
...
void solution(struct Node *root) 
{
    ...
    {
        solution(root->left);                 // Traversing through left subtree
        solution(root->right);                // Traversing through right subtree
        printf("%d ",root->key);             // Printing the last element
    }
}
